...
class inputaboutus extends CI_Controller {
    ...
    public function index($xstart = 0, $xSearch = '') {
        ...
        if (!empty($idpegawai)) {
            ...
            $head['ajax'] = '<script language="javascript" type="text/javascript" src="' . base_url() . 'ajax/ajaxinputaboutus.js"></script>' . "\n" .
...
//        $idx = $_POST['idx'];
        $row = $this->modelaboutus->getorderdetailaboutus(0, 1, 'DESC', 1);
        ...
        $xcontent.='<input type="hidden" id="images_slider" value=""/>';
